...
def fc_tree_commute_recur_lookahead_to_start(entanglers: List[str], params: List[float], barrier=False, threshold=1):
    '''This function defines the optimized fully connected tree block for Hamiltonian simulation in commute list format, 
    commuting the CNOT trees to the beginning of the circuit.
    
    Args:
        entanglers: list storing Pauli words for construction of the optimized qcc_circuit.
        params: parameters for the rotations
        barrier: barriers between blocks of gates
        threshold: lookahead threshold for the optimization process
    Returns:
        opt_qc, append_clifford, opt_paulis, opt_params
    '''
    commute_sets = convert_commute_sets(Paulis=entanglers, params=params)
    ...
    # Sort all the Pauli strings based on their weight
    for commute_list in commute_sets:
        sorted_list = sorted(commute_list, key=lambda x: pauli_weight(x[0]))
        sorted_entanglers_params.append(sorted_list)

    # Reverse the main loop to process the Pauli strings from the last commuting set to the first
    for commute_idx in reversed(range(len(sorted_entanglers_params))):
        sorted_list = sorted_entanglers_params[commute_idx]
        
        for pauli_idx in reversed(range(len(sorted_list))):
            curr_pauli = sorted_list[pauli_idx][0]
            curr_param = sorted_list[pauli_idx][1]
            
            sq_qc = construct_sq_subcircuit(curr_pauli)  # Construct the single-qubit subcircuit
            
            # The next Pauli is now determined by looking backwards
            if pauli_idx == 0:
                if commute_idx == 0:
                    next_pauli = None
                else:
                    if len(sorted_entanglers_params[commute_idx - 1]) > 1:
                        next_pauli_index = find_best_pauli_index_threshold(
                            base_entangler=curr_pauli, 
                            commute_sets=sorted_entanglers_params[commute_idx - 1], 
                            append_clifford=append_clifford, 
                            threshold=threshold
                        )
                        element = sorted_entanglers_params[commute_idx - 1].pop(next_pauli_index)
                        sorted_entanglers_params[commute_idx - 1].insert(0, element)
                ...
                next_pauli_index = find_best_pauli_index_threshold(
                    base_entangler=curr_pauli, 
                    commute_sets=sorted_entanglers_params[commute_idx][:pauli_idx], 
                    append_clifford=append_clifford, 
                    threshold=threshold
                )
                element = sorted_entanglers_params[commute_idx].pop(next_pauli_index)
                sorted_entanglers_params[commute_idx].insert(0, element)
            
            if next_pauli is None:
                extracted_clif = construct_Clifford_subcircuit(curr_pauli)
                append_clifford = extracted_clif.compose(append_clifford)
                extracted_qc = construct_extracted_subcircuit(entangler=curr_pauli, param=curr_param)
                ...
                if barrier:
                    opt_qc.barrier()
                opt_qc = extracted_qc.compose(opt_qc)  # Add to the beginning instead of the end
            else:
                init_cx_tree = QuantumCircuit(len(entanglers[0]))
                next_commute_idx, next_pauli_idx = gen_next_pauli_idx(sorted_entanglers_params, commute_idx, pauli_idx)
                tree_list = [len(curr_pauli) - 1 - i for i in range(len(curr_pauli)) if curr_pauli[i] != 'I']
                sq_index = find_leaves(
                    sorted_entanglers_params, curr_pauli=curr_pauli, 
                    updated_paulis={}, qc_tree=init_cx_tree, tree_list=tree_list, 
                    commute_idx=next_commute_idx, pauli_idx=next_pauli_idx, 
                    append_clifford=append_clifford
                )
                init_clif = sq_qc.inverse()
                init_clif.compose(init_cx_tree, inplace=True)
                ...
                if barrier:
                    opt_qc.barrier()
                opt_qc = init_clif.compose(opt_qc)  # Compose at the start

                if pauli_idx == 0:
                    sorted_entanglers_params[commute_idx - 1][0] = update_pauli_param(
                        Pauli_param=sorted_entanglers_params[commute_idx - 1][0], 
                        clifford_circuit=append_clifford
                    )
                else:
                    sorted_entanglers_params[commute_idx][pauli_idx - 1] = update_pauli_param(
                        Pauli_param=sorted_entanglers_params[commute_idx][pauli_idx - 1], 
                        clifford_circuit=append_clifford
                    )
    return opt_qc, append_clifford, sorted_entanglers_params, commute_sets
